...
namespace Mfc.Network
{
    public class LocalNode : IDisposable
    {
        ...
        public LocalNode()
        {
            ...
            this.UserAgent = string.Format("/MFC:{0}/", Assembly.GetExecutingAssembly().GetVersion());
                ...
                Task.Run(async () =>
                {
                        ...
                        && await UPnP.DiscoverAsync())
                    {
                        try
                        {
                            ...
                            if (port > 0)
                                await UPnP.ForwardPortAsync(port, ProtocolType.Tcp, "MFC");
                            if (ws_port > 0)
                                await UPnP.ForwardPortAsync(ws_port, ProtocolType.Tcp, "MFC WebSocket");
                        }
                        catch { }
                    }
                    connectThread.Start();
